...
class TestProjectModel(TestCase):
    ...
    def test_validation_color(self):
        # This first project uses default value #fff
        project = models.Project(
            user=self.profile,
            name="TaskManager"
        )
        self.assertTrue(project.color == "#fff")
        # Validation shouldn't throw error
        project.full_clean()
        ...
        # Bad color inputs
        vals = ["1cA", "1256aB", "#1", "#12", "#1234", "#12345", "#1234567"]
        for color in vals:
            with self.assertRaises(
                    ValidationError,
                    msg="{} didn't raise a ValiationError".format(color)):
                project.color = color
                project.full_clean()
